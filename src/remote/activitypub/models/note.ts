...
import { INote as INoteActivityStreamsObject, IObject, getASIds, getOneASId, getASId } from '../type';
...
export function validateNote(object: any, uri: string) {
	...
	if (object.attributedTo && extractApHost(getOneASId(object.attributedTo)) !== expectHost) {
...
export async function createNote(value: string | IObject, resolver?: Resolver, silent = false): Promise<INote> {
	...
	// 投稿者をフェッチ
	const actor = await resolvePerson(getOneASId(note.attributedTo), null, resolver) as IRemoteUser;
	...
	//#region Visibility
	note.to = getASIds(note.to);
	note.cc = getASIds(note.cc);
