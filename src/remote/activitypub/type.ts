 ...
 */
export function getASIds(value: string[] | string | IObject[] | IObject): string[] {
	if (value == null) return [];
	const array = Array.isArray(value) ? value : [value];
	return array.map(x => getASId(x));
}

/**
 * Get first ActivityStreams Object id
 */
export function getOneASId(value: string[] | string | IObject[] | IObject): string {
	const firstOne = Array.isArray(value) ? value[0] : value;
	return getASId(firstOne);
}

/**
 * Get ActivityStreams Object id
 */
export function getASId(value: string | IObject): string {
